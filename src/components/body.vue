        ...
        <div class="events-week" v-for="week in currentDates">
          <div class="events-day" v-for="day in week" track-by="$index"
            ...
            <div class="event-box">
              ...
              <p class="event-item" v-if="day.events.length == eventLimit"
                :class="[classNames(day.events[eventLimit-1].cssClass), {
                  'is-start'   : isStart(day.events[eventLimit-1].start, day.date),
                  'is-end'     : isEnd(day.events[eventLimit-1].end,day.date),
                  'is-opacity' : !day.events[eventLimit-1].isShow
                  }]" 
                @click="eventClick(day.events[eventLimit-1],$event)">
                {{isBegin(day.events[eventLimit-1], day.date, day.weekDay)}}
              </p>
              <p v-if="day.events.length > eventLimit"
                class="more-link" @click.stop="selectThisDay(day, $event)">
                ※ {{eventLimit}}件以上予定があります
              </p>
            </div>
          </div>
        </div>
